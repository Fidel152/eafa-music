...
export default function Rehearsals() {
  ...
  const openRehearsalDetails = async (rehearsal: Rehearsal) => {
    setSelectedRehearsal(rehearsal);
    setIsModalOpen(false); // Ensure planning modal is closed when viewing details
    
    // Always trigger tracking when opening details
    if (user) {
      try {
        await api.rehearsals.trackView(rehearsal.id, user.id);
        // If admin, reload viewers after tracking to see latest state
        if (isAdmin) {
          loadViewers(rehearsal.id);
        }
      } catch (error) {
        console.error("Rehearsal tracking error:", error);
      }
    }
  };

  const loadViewers = async (rehearsalId: string) => {
    setLoadingViewers(true);
    try {
      const data = await api.rehearsals.getViewers(rehearsalId);
      setViewers(data || []);
    } catch (e) {
      console.error("Load rehearsal viewers error:", e);
    } finally {
            ...
            {rehearsals.map((rehearsal) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0" onClick={() => openRehearsalDetails(rehearsal)}>
                  ...
                  <div className="flex flex-col gap-3 w-full md:w-auto">
                    ...
                    {isAdmin && (
                      <div className="flex items-center gap-2 mt-2">
                        <button 
                          onClick={() => openRehearsalDetails(rehearsal)}
                    ...
                    {!isAdmin && (
                       <button 
                         onClick={() => openRehearsalDetails(rehearsal)}
                         className="w-full mt-2 py-2 text-xs font-bold text-[#D4AF37] border border-[#D4AF37]/20 rounded-xl hover:bg-[#D4AF37]/5"
                       >
                         Voir détails & notes
                       </button>
                    )}
        ...
        {selectedRehearsal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="bg-[#002B5B] p-6 text-white flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-bold">{isAdmin ? 'Présences :' : 'Détails :'} {selectedRehearsal.title}</h2>
              ...
              <div className="p-0 overflow-y-auto flex-1 pb-8">
                {selectedRehearsal.description && (
                  <div className="p-6 bg-slate-50 border-b border-slate-100">
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Notes et instructions</p>
                    <p className="text-slate-700 leading-relaxed text-sm md:text-base">{selectedRehearsal.description}</p>
                  </div>
                )}
                  ...
                  <div className="p-0">
                    ...
                    <div className="mt-8 px-6 pt-6 border-t border-slate-100">
                      ...
                      {loadingViewers ? (
                        ...
                        <p className="text-slate-400 text-xs italic pb-8">Personne n'a encore vu ces détails.</p>
                      )}
                    </div>
                  </div>
                )}
